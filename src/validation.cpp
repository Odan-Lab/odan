...
std::unique_ptr<OdanState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        OdanDGP odanDGP(globalState.get(), m_active_chainstate, fGettingValuesDGP);
        uint64_t minGasPrice = odanDGP.getMinGasPrice(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        uint64_t blockGasLimit = odanDGP.getBlockGasLimit(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        size_t count = 0;
        ...
        OdanTxConverter converter(tx, m_active_chainstate, &m_pool, NULL, NULL, contractflags);
        ExtractOdanTX resultConverter;
        if(!converter.extractionOdanTransactions(resultConverter)){
        ...
        std::vector<OdanTransaction> odanTransactions = resultConverter.first;
        std::vector<EthTransactionParams> odanETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(OdanTransaction odanTransaction : odanTransactions){
            sumGas += odanTransaction.gas() * odanTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, odanTransaction.gasPrice());
            } else {
                txMinGasPrice = odanTransaction.gasPrice();
            }
            VersionVM v = odanTransaction.getVersion();
            if(v.format!=0)
            ...
            if(odanTransaction.gas() < gArgs.GetIntArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(odanTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(odanTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)odanTransaction.gasPrice() < minGasPrice)
...
static constexpr uint64_t FP_SCALE = 100000000ULL;

CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    // Phase 0: Genesis (height 0)
    // Phase 1: High-subsidy window (post-genesis premine-by-mining)
    // Heights 1..highSubsidyEndHeight pay a large constant amount (2000 COIN).
    if (nHeight > 0 && nHeight <= consensusParams.nLastBigReward) {
        return consensusParams.highSubsidyAmount;
    }

    // Phase 2: Baseline schedule (10 COIN decaying by 1/1.1149 every 210,240 blocks)
    const int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;

    // Use precomputed values when possible
    if (halvings < (int)consensusParams.subsidyLookup.size())
        return consensusParams.subsidyLookup[halvings];

    // Continue fixed-point decay beyond the lookup table
    CAmount v = consensusParams.subsidyLookup.back();
    for (int i = (int)consensusParams.subsidyLookup.size(); i <= halvings && v > 0; ++i) {
        v = (v * (CAmount)FP_SCALE) / (CAmount)consensusParams.decayFactorFP;
    }
    return v;
}
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // odan
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // odan
    ...
    if(pindex->nHeight >= chainparams.GetConsensus().nFirstMPoSBlock)
    {
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, Chainstate& chainstate, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    OdanDGP odanDGP(globalState.get(), chainstate, fGettingValuesDGP);
    uint64_t blockGasLimit = odanDGP.getBlockGasLimit(chainstate.m_chain.Tip()->nHeight + 1);
    ...
    OdanTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = OdanTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = OdanTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<OdanTransaction>(1, callTransaction), blockGasLimit, pblockindex, chainstate.m_chain);
...
void writeVMlog(const std::vector<ResultExecute>& res, CChain& chain, const CTransaction& tx, const CBlock& block){
    fs::path odanDir = gArgs.GetDataDirNet() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(PathToString(odanDir), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(PathToString(odanDir), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    ...
    for(OdanTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, OdanTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
dev::eth::EnvInfo ByteCodeExec::BuildEVMEnvironment(){
    ...
    chainID = odanutils::eth_getChainId(tip->nHeight);
...
bool OdanTxConverter::extractionOdanTransactions(ExtractOdanTX& odantx){
    ...
    std::vector<OdanTransaction> resultTX;
    ...
    odantx = std::make_pair(resultTX, resultETP);
    return true;
}

bool OdanTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool OdanTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
OdanTransaction OdanTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    OdanTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = OdanTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = OdanTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t OdanTxConverter::correctedStackSize(size_t size){
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // odan
    OdanDGP odanDGP(globalState.get(), *this, fGettingValuesDGP);
    globalSealEngine->setOdanSchedule(odanDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = odanDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = odanDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = odanDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || GetSerializeSize(TX_NO_WITNESS(block)) > dgpMaxBlockSize) // odan
...
///////////////////////////////////////////////////////////////////////////////////////// odan
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            OdanTxConverter convert(tx, *this, m_mempool, &view, &block.vtx, contractflags);

            ExtractOdanTX resultConvertOdanTX;
            if(!convert.extractionOdanTransactions(resultConvertOdanTX)){
            ...
            if(!CheckMinGasPrice(resultConvertOdanTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertOdanTX.first, blockGasLimit, pindex->pprev, m_chain);
            ...
            for(OdanTransaction& qtx : resultConvertOdanTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertOdanTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertOdanTX.first[k].from(),
                        resultConvertOdanTX.first[k].to(),
                        ...
                        resultConvertOdanTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    // The stake and delegate index is needed for MPoS, update it while MPoS is active
    if(pindex->nHeight >= params.GetConsensus().nFirstMPoSBlock)
    {
        if(block.IsProofOfStake()){
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // odan
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // odan
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // odan
            globalState->setRootUTXO(oldHashUTXORoot); // odan
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // odan
        uint32_t sizeBlockDGP = odanDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Odan PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx, const CTxMemPool& mempool, Chainstate& active_chainstate)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        OdanTxConverter convert(tx, active_chainstate, &mempool, NULL, NULL, contractflags);
