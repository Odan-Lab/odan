...
//    initState();
    ...
    txs.push_back(createOdanTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createOdanTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    ...
    txsCall.push_back(createOdanTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall, *m_node.chainman);

    // Call mint
    std::vector<OdanTransaction> txsMint;
    txsMint.push_back(createOdanTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    ...
    // Call balance of
    std::vector<OdanTransaction> txsbalance;
    txsbalance.push_back(createOdanTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs, *m_node.chainman);

    // Call is it constantinople
    dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<OdanTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createOdanTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
