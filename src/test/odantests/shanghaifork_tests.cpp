...
void checkChainid(dev::h256& hashTx, ChainstateManager& chainman, const ChainType chain, const odanutils::ChainIdType& chainId)
{
    // Select params
    ...
    txs.push_back(createOdanTransaction(getCode(CodeID::chainIdcontract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...

    {
        ...
        std::vector<OdanTransaction> txShanghai;
        txShanghai.push_back(createOdanTransaction(getCode(CodeID::getChainId), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
{
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(getCode(codeId), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...
BOOST_AUTO_TEST_CASE(checking_shanghai_after_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, ChainType::MAIN, odanutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, ChainType::TESTNET, odanutils::ChainIdType::TESTNET);
    checkChainid(hashTx, *m_node.chainman, ChainType::REGTEST, odanutils::ChainIdType::REGTEST);
...
BOOST_AUTO_TEST_CASE(checking_shanghai_before_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, ChainType::MAIN, odanutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, ChainType::TESTNET, odanutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, ChainType::REGTEST, odanutils::ChainIdType::MAIN);
