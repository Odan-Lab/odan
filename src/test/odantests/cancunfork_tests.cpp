...
BOOST_AUTO_TEST_CASE(checking_transient_storage_after_fork){
    ...
    txs.push_back(createOdanTransaction(getCode(CodeID::transientStorageContract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    ...
    std::vector<OdanTransaction> txCancun;
    txCancun.push_back(createOdanTransaction(getCode(CodeID::setMultiplier_5), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    txCancun.push_back(createOdanTransaction(getCode(CodeID::multiply_7), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
BOOST_AUTO_TEST_CASE(checking_blobbasefee_after_fork){
    ...
    // Create contract
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(getCode(CodeID::blobBaseFeeContract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    auto result = executeBC(txs, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<OdanTransaction> txCancun;
    txCancun.push_back(createOdanTransaction(getCode(CodeID::getBlobBaseFee), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    result = executeBC(txCancun, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
...
BOOST_AUTO_TEST_CASE(checking_memory_copy_after_fork){
    ...
    // Create contract
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(getCode(CodeID::memoryCopyContract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    auto result = executeBC(txs, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<OdanTransaction> txCancun;
    txCancun.push_back(createOdanTransaction(getCode(CodeID::memoryCopy_80), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    result = executeBC(txCancun, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
...
BOOST_AUTO_TEST_CASE(checking_blob_hash_after_fork){
    ...
    // Create contract
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(getCode(CodeID::emptyBlobContract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    auto result = executeBC(txs, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<OdanTransaction> txCancun;
    txCancun.push_back(createOdanTransaction(getCode(CodeID::getBlobHash_7), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    result = executeBC(txCancun, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
...
BOOST_AUTO_TEST_CASE(checking_point_evaluation_after_fork){
    ...
    // Create contract
    std::vector<OdanTransaction> txs;
    txs.push_back(createOdanTransaction(getCode(CodeID::pointEvaluationContract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    auto result = executeBC(txs, *m_node.chainman);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<OdanTransaction> txCancun;
    txCancun.push_back(createOdanTransaction(getCode(CodeID::verifyKzgCorrectProof), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    txCancun.push_back(createOdanTransaction(getCode(CodeID::verifyKzgIncorrectProof), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    result = executeBC(txCancun, *m_node.chainman);
