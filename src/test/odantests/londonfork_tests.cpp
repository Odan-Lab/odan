...
#include <odantests/precompiled_utils.h>
#include <test/odantests/data/btc_ecrecover.json.h>
#include <test/odantests/data/ecrecover.json.h>
#include <test/odantests/data/sha256.json.h>
#include <test/odantests/data/ripemd160.json.h>
#include <test/odantests/data/identity.json.h>
#include <test/odantests/data/modexp.json.h>
#include <test/odantests/data/modexp_eip2565.json.h>
#include <test/odantests/data/alt_bn128_G1_add.json.h>
#include <test/odantests/data/alt_bn128_G1_mul.json.h>
#include <test/odantests/data/alt_bn128_pairing_product.json.h>
#include <test/odantests/data/blake2_compression.json.h>
...
BOOST_AUTO_TEST_CASE(checking_london_after_fork){
    ...
    txs.push_back(createOdanTransaction(getCode(CodeID::contract), 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs, *m_node.chainman);

    {
        // Call basefee
        assert(txs.size() > 0);
        dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<OdanTransaction> txIsItLondon;
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::getBaseFee), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...

    {
        ...
        dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<OdanTransaction> txIsItLondon;
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::deploy1ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::deploy2ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::deploy3ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::deploy32ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::deploy1fe), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::call1ef), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::call2ef), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
...

    {
        ...
        dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<OdanTransaction> txIsItLondon;
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::getStore), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::getLoad), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
        BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    for(size_t a = 1; a < 300; a++)
    {
        ...
        dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<OdanTransaction> txIsItLondon;
        txIsItLondon.push_back(createOdanTransaction(data, 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
...

    {
        ...
        dev::Address proxy = createOdanAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<OdanTransaction> txIsItLondon;
        txIsItLondon.push_back(createOdanTransaction(getCode(CodeID::close), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
