#ifndef ODANTESTS_TEST_UTILS_H
#define ODANTESTS_TEST_UTILS_H
...
extern std::unique_ptr<OdanState> globalState;
...
inline void initState(){
    ...
    const std::string dirOdan = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<OdanState>(new OdanState(dev::u256(0), OdanState::openDB(dirOdan, hashDB, dev::WithExisting::Trust), dirOdan + "/odanDB", dev::eth::BaseState::Empty));
...
inline dev::Address createOdanAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline OdanTransaction createOdanTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    OdanTransaction txEth;
    if(recipient == dev::Address()){
        txEth = OdanTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = OdanTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<OdanTransaction> txs, ChainstateManager& chainman){
    CBlock block(generateBlock());
    OdanDGP odanDGP(globalState.get(), chainman.ActiveChainstate(), fGettingValuesDGP);
    uint64_t blockGasLimit = odanDGP.getBlockGasLimit(chainman.ActiveChain().Tip()->nHeight + 1);
