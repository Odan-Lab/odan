...
namespace odantxconverter_tests {
//Tests data
...
void checkResult(bool isCreation, std::vector<OdanTransaction> results, uint256 hash){
...
void runTest(Chainstate& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    OdanTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractOdanTX odanTx;
    BOOST_CHECK(converter.extractionOdanTransactions(odanTx));
    std::vector<OdanTransaction> result = odanTx.first;
    if(script2 == CScript()){
...
using namespace odantxconverter_tests;

BOOST_FIXTURE_TEST_SUITE(odantxconverter_tests, TestingSetup)
