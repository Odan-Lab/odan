...
public:
    ...
    void getGasInfo(uint64_t& blockGasLimit, uint64_t& minGasPrice, uint64_t& nGasPrice) override
    {
        ...
        blockGasLimit = odanDGP.getBlockGasLimit(numBlocks);
        minGasPrice = CAmount(odanDGP.getMinGasPrice(numBlocks));
...
public:
    ...
    bool getDelegation(const uint160& address, Delegation& delegation) override
    {
        ...
        return odanDelegation.ExistDelegationContract() ? odanDelegation.GetDelegation(address, delegation, chainman().ActiveChainstate()) : false;
    }
    bool verifyDelegation(const uint160& address, const Delegation& delegation) override
    {
        return OdanDelegation::VerifyDelegation(address, delegation);
    }
    NodeContext& m_node;
};
} // namespace
