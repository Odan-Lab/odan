    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    fs::path odanStateDir = gArgs.GetDataDirNet() / "stateOdan";
    bool fStatus = fs::exists(odanStateDir);
    const std::string dirOdan = PathToString(odanStateDir);
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    dev::eth::BaseState existsOdanstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
    globalState = std::unique_ptr<OdanState>(new OdanState(dev::u256(0), OdanState::openDB(dirOdan, hashDB, dev::WithExisting::Trust), dirOdan, existsOdanstate));
    const CChainParams& chainparams = Params();
    dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
    globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

    pstorageresult.reset(new StorageResults(PathToString(odanStateDir)));
    if (options.reindex) {
...
ChainstateLoadResult VerifyLoadedChainstate(ChainstateManager& chainman, const ChainstateLoadOptions& options)
{
    ...
    globalSealEngine->setOdanSchedule(odanDGP.getGasSchedule(active_chain.Height() + (active_chain.Height()+1 >= chainman.GetConsensus().QIP7Height ? 0 : 1) ));
