...
void updateMinerParams(int nHeight, const Consensus::Params& consensusParams, bool minDifficulty)
{
    ...
    unsigned int timeDownscaleTmp = 1;
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    //////////////////////////////////////////////////////// odan
    OdanDGP odanDGP(globalState.get(), m_chainstate, fGettingValuesDGP);
    globalSealEngine->setOdanSchedule(odanDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = odanDGP.getBlockSize(nHeight);
    minGasPrice = odanDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = odanDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice, CBlock* pblock) {
    ...
    OdanTxConverter convert(iter->GetTx(), m_chainstate, m_mempool, NULL, &pblock->vtx, contractflags);

    ExtractOdanTX resultConverter;
    if(!convert.extractionOdanTransactions(resultConverter)){
    ...
    for(OdanTransaction odanTransaction : odanTransactions){
        ...
        if(bceResult.usedGas + odanTransaction.gas() > softBlockGasLimit){
        ...
        if(odanTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, odanTransactions, hardBlockGasLimit, m_chainstate.m_chain.Tip(), m_chainstate.m_chain);
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            odanDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
            delegations_staker = odanDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                odanDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                odanDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            odanDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            odanDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    ...
    OdanDelegation odanDelegations;
    int32_t cacheHeight;
    ...
    void Update(int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                odanDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
                pwallet->m_my_delegations = odanDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    odanDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                    odanDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                odanDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(odanDelegations.GetDelegation(address, delegation, pwallet->chain().chainman().ActiveChainstate()) && OdanDelegation::VerifyDelegation(address, delegation))
                    {
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, wallet::CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    OdanLedger &device = OdanLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, wallet::CWallet& wallet)
{
    ...
    {
        std::string errorMessage = OdanLedger::instance().errorMessage();
...
public:
...

    {
        ...
        std::string threadName = "odanstake";
...
void StakeOdans(bool fStake, wallet::CWallet *pwallet)
{
