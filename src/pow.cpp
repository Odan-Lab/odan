...
{
    // Odan has different difficulty adjustment algorithm than Bitcoin, so checking the borders for the new difficulty value might not be the same.
...
bool PermittedDifficultyTransition(const Consensus::Params& params, int64_t height, uint32_t old_nbits, uint32_t new_nbits)
{
    ...
    if (height % params.DifficultyAdjustmentInterval(height) == 0) {
        int64_t smallest_timespan = params.nPowTargetTimespan;
