...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= 10 * COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 10ODAN/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> odan-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> odan-cli -named " + methodname;
