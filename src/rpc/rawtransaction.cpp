                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in odan-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in odan-common, so we query them here and push the
    ...
    if(isExpanded)
    {
        if(isVin) {
            return {
                ...
                {RPCResult::Type::STR, "address", /*optional=*/true, "The Odan address (only if address index is enabled)"},
            };
        }
        else {
            return {
                ...
                {"contract", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "(send to contract)",
                    {
                        ...
                        {"amount", RPCArg::Type::AMOUNT,  RPCArg::Default{0}, "Value in ODAN to send with the call, should be a valid amount, default 0"},
                        ...
                        {"senderAddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "The odan address that will be used to create the contract."},
                    },
                },
...
public:
    ...
    void addContract(std::vector<std::pair<CTxDestination, CAmount>>& parsed_outputs, const UniValue& Contract) override
    {
        ...
        uint64_t blockGasLimit = odanDGP.getBlockGasLimit(active_chain.Height());
        uint64_t minGasPrice = CAmount(odanDGP.getMinGasPrice(active_chain.Height()));
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Odan transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed Odan transactions into one transaction.\n"
