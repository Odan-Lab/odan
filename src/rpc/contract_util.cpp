...
UniValue transactionReceiptToJSON(const OdanTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params, ChainstateManager &chainman)
{
    ...
    if(!params[2].isNull()){
        CTxDestination odanSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(odanSenderAddress)) {
            PKHash keyid = std::get<PKHash>(odanSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToOdanAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToOdanAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
