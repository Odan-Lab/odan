...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "odan-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                auto init = ipc->spawnProcess("odan-node");
                echo = init->makeEcho();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    OdanDGP odanDGP(globalState.get(), chainman.ActiveChainstate());

    UniValue obj(UniValue::VOBJ);
    obj.pushKV("maxblocksize", (uint64_t)odanDGP.getBlockSize(active_chain.Height()));
    obj.pushKV("mingasprice", (uint64_t)odanDGP.getMinGasPrice(active_chain.Height()));
    obj.pushKV("blockgaslimit", (uint64_t)odanDGP.getBlockGasLimit(active_chain.Height()));
    ...
    return RPCHelpMan{"getaddressdeltas",
            "\nReturns all changes for an address (requires addressindex to be enabled).\n",
            {
                {"argument", RPCArg::Type::OBJ, RPCArg::Optional::NO, "Json object",
                    {
                        {"addresses", RPCArg::Type::ARR, RPCArg::Optional::NO, "The odan addresses",
                            {
                                {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "The odan address"},
                            }
                        },
...
static RPCHelpMan listconf()
{
    return RPCHelpMan{"listconf",
                "\nReturns the current options that odand was started with.\n",
                {},
                RPCResult{
                    RPCResult::Type::OBJ_DYN, "", "",
                    {
