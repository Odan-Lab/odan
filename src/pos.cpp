...
OdanDelegation& GetOdanDelegation()
{
    static OdanDelegation odanDelegation;
    return odanDelegation;
}
...
bool CheckStakeKernelHash(CBlockIndex* pindexPrev, unsigned int nBits, uint32_t blockFromTime, CAmount prevoutValue, const COutPoint& prevout, unsigned int nTimeBlock, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    bool fNoBNOverflow = nHeight >= Params().GetConsensus().QIP9Height;
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view, Chainstate& chainstate)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        OdanDelegation& odanDelegation = GetOdanDelegation();
        if(!odanDelegation.ExistDelegationContract())
        ...
        if(!odanDelegation.GetDelegation(address, delegation, chainstate)) {
        ...
        bool verifiedDelegation = odanDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee, Chainstate& chainstate)
{
    Delegation delegation;
    OdanDelegation& odanDelegation = GetOdanDelegation();
    bool ret = odanDelegation.GetDelegation(address, delegation, chainstate);
    if(ret) ret &= odanDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
