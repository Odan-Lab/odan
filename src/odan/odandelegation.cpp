...
bool OdanDelegation::GetDelegation(const uint160 &address, Delegation &delegation, Chainstate& chainstate) const
{
...
bool OdanDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool OdanDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, ChainstateManager &chainman, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> OdanDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void OdanDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool OdanDelegation::ExistDelegationContract() const
{
    LOCK(cs_main);
...
std::string OdanDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool OdanDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool OdanDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool OdanDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool OdanDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
