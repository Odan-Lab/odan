...
using namespace OdanLedger_NS;

class OdanLedgerPriv
{
public:
    OdanLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("OdanLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
OdanLedger::OdanLedger():
    d(0)
{
    d = new OdanLedgerPriv();
}
...
OdanLedger &OdanLedger::instance()
{
    static OdanLedger device;
    return device;
}

bool OdanLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool OdanLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool OdanLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, bool descriptorwallet, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::displayAddress(const std::string &fingerprint, const std::string &desc, std::string &address)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool OdanLedger::displayAddress(const std::string &fingerprint, int type, const std::string &path, std::string &address)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string OdanLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool OdanLedger::toolExists()
{
    return d->toolExists;
}

bool OdanLedger::isStarted()
{
    return d->fStarted;
}

void OdanLedger::wait()
{
    if(d->fStarted)
    {
...
bool OdanLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool OdanLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool OdanLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool OdanLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool OdanLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool OdanLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Odan Stake" || device.app_name == "Odan Stake Test";
        if(isStakeApp == stake)
        {
...
bool OdanLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool OdanLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool OdanLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, bool, std::string &)
{
    // Get the output type
...
bool OdanLedger::endGetKeyPool(const std::string &, int type, const std::string& , bool, int, int, bool descriptorwallet, std::string &desc)
{
    // Decode command line results
...
bool OdanLedger::beginDisplayAddress(const std::string &fingerprint, int type, const std::string& path)
{
    // Get the output type
    std::string descType = get_address_type(type);
...
bool OdanLedger::beginDisplayAddress(const std::string &fingerprint, const std::string &desc)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool OdanLedger::endDisplayAddress(std::string &address)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
std::string OdanLedger::derivationPath(int type)
{
