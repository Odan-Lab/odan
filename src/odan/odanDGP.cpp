...
void OdanDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool OdanDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule OdanDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t OdanDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t OdanDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t OdanDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t OdanDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool OdanDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void OdanDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void OdanDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void OdanDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data, chainstate)[0].execRes.output;
}

void OdanDGP::createParamsInstance(){
...
dev::Address OdanDGP::getAddressForBlock(unsigned int blockHeight){
...
void OdanDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OdanDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OdanDGP::parseStorageOneUint64(uint64_t& value){
...
void OdanDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule OdanDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
