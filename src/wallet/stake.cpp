...
void StakeOdans(CWallet& wallet, bool fStake)
{
    node::StakeOdans(fStake, &wallet);
}
...
void StartStake(CWallet& wallet)
{
    ...
    {
        wallet.m_enabled_staking = node::ENABLE_HARDWARE_STAKE && !wallet.m_ledger_id.empty() && OdanLedger::instance().toolExists();
    }
    else
    {
