                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::ARR, "transactions", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", /*optional=*/true, "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address",  /*optional=*/true, "The odan address of the transaction (not returned if the output does not have an address, e.g. OP_RETURN null data)."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase and non-coinstake transactions received.\n"
                                    "\"generate\"              Coinbase or coinstake transactions received with more than 100 confirmations.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request_) -> UniValue
{
    ...
    if (!shouldWaitConf) {
        ...
        if(!request.httpreq)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "No HTTP connection. Waitconf is available from odan-cli, not odan-qt");
