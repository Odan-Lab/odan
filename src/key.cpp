...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig, bool grind, uint32_t test_case) const {
    ...
    const unsigned char* seckey = UCharCast(begin());
    if (!IsValid() || seckey == nullptr) {
        return false; // key not initialized or invalid
    }

    // Verify secret key is valid for secp256k1
    if (!secp256k1_ec_seckey_verify(secp256k1_context_sign, seckey)) {
        return false; // zeroized or out-of-range key
    }
    ...
    // Grind for low-R signatures if requested
    while (grind && !SigHasLowR(&sig)) {
        WriteLE32(extra_entropy, ++counter);
    ...
    size_t nSigLen = CPubKey::SIGNATURE_SIZE;
    vchSig.resize(nSigLen);
    if (!secp256k1_ecdsa_signature_serialize_der(secp256k1_context_sign, vchSig.data(), &nSigLen, &sig)) {
        return false;
    }
    vchSig.resize(nSigLen);

    // Verify signature with a proper verify context
    secp256k1_pubkey pk;
    if (!secp256k1_ec_pubkey_create(secp256k1_context_sign, &pk, seckey)) {
        return false;
    }
    if (!secp256k1_ecdsa_verify(secp256k1_context_static, &sig, hash.begin(), &pk)) {
        return false;
    }
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    std::string str = "Odan key verification\n";
    GetRandBytes(rnd);
