...
#define paternVersion "odan-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

OdanVersionChecker::OdanVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, 0);
}

OdanVersionChecker::~OdanVersionChecker()
{

}

bool OdanVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> OdanVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(ODAN_RELEASES)));
    QTimer timer;
...
Version OdanVersionChecker::getMaxReleaseVersion()
{
