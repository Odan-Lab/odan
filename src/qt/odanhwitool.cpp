...
#include <qt/odanhwitool.h>
#include <qt/guiutil.h>
#include <qt/execrpccommand.h>
#include <qt/walletmodel.h>
#include <util/strencodings.h>
#include <common/system.h>
#include <odan/odanledger.h>
...
class OdanHwiToolPriv
{
public:
    OdanHwiToolPriv(QObject *parent)
    {
...
OdanHwiTool::OdanHwiTool(QObject *parent) : QObject(parent)
{
    d = new OdanHwiToolPriv(this);
}

OdanHwiTool::~OdanHwiTool()
{
    delete d;
}

bool OdanHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(OdanLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(OdanLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool OdanHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = OdanLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(OdanLedger::instance().errorMessage());
    return ret;
}

bool OdanHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = OdanLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, isDescriptorWallet(), strDesc);
...
bool OdanHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool OdanHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool OdanHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool OdanHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool OdanHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = OdanLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(OdanLedger::instance().errorMessage());
    return ret;
}

bool OdanHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = OdanLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(OdanLedger::instance().errorMessage());
    return ret;
}

bool OdanHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
bool OdanHwiTool::displayAddress(const QString &fingerprint, const QString &desc, QString &address)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strDesc = desc.toStdString();
    std::string strAddress;
    bool ret = OdanLedger::instance().displayAddress(strFingerprint, strDesc, strAddress);
    address = QString::fromStdString(strAddress);
    if(!ret) d->strError = QString::fromStdString(OdanLedger::instance().errorMessage());
    return ret;
}

QString OdanHwiTool::errorMessage()
{
...
bool OdanHwiTool::isStarted()
{
    return d->fStarted;
}

void OdanHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool OdanHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool OdanHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool OdanHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool OdanHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool OdanHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool OdanHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
bool OdanHwiTool::getAddressDesc(const QString &address, QString &desc)
{
    if(!d->model) return false;
...
void OdanHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool OdanHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void OdanHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
bool OdanHwiTool::isDescriptorWallet()
{
    if(!d->model) return false;
    return d->model->wallet().hasDescriptors();
}

QString OdanHwiTool::derivationPathPKH()
{
    std::string path = OdanLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString OdanHwiTool::derivationPathP2SH()
{
    std::string path = OdanLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString OdanHwiTool::derivationPathBech32()
{
    std::string path = OdanLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
