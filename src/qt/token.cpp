...
Token::Token()
{
    ...
    lstMandatory.append(OdanToken::paramAddress());
    lstMandatory.append(OdanToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(OdanToken::paramSender());
    ...
    lstMandatory.append(OdanToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(OdanToken::paramAmount());
    lstOptional.append(OdanToken::paramGasLimit());
    lstOptional.append(OdanToken::paramGasPrice());
    lstOptional.append(OdanToken::paramSender());
    lstOptional.append(OdanToken::paramBroadcast());
    lstOptional.append(OdanToken::paramChangeToSender());
    lstOptional.append(OdanToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToOdanAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToOdanAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
