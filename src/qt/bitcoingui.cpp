...
#include <qt/odanversionchecker.h>
    ...
    m_network_style(networkStyle)
{
   ...
   if(enableWallet)
    ...
    odanVersionChecker = new OdanVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and odan: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Odan addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Odan addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Odan Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Odan Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a odan: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Odan command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && odanVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(ODAN_RELEASES, ODAN_RELEASES);
            QString message(tr("New version of Odan wallet is available on the Odan source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Odan network.", "", count);
    } else {
        //: A substring of the tooltip.
