#include <qt/oastoken.h>
#include <qt/forms/ui_oastoken.h>
...
OASToken::OASToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OASToken),
    m_model(0),
    m_clientModel(0),
    m_tokenTransactionView(0)
{
    ...
    connect(m_tokenList, &TokenListWidget::sendToken, this, &OASToken::on_sendToken);
    connect(m_tokenList, &TokenListWidget::receiveToken, this, &OASToken::on_receiveToken);
    connect(m_tokenList, &TokenListWidget::addToken, this, &OASToken::on_addToken);
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &OASToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &OASToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &OASToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &OASToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &OASToken::removeToken);

    connect(m_tokenList, &TokenListWidget::customContextMenuRequested, this, &OASToken::contextualMenu);

    connect(m_sendTokenPage, &SendTokenPage::message, this, &OASToken::message);
}
...
void OASToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    {
        // Set current token
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::dataChanged, this, &OASToken::on_dataChanged);
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::rowsInserted, this, &OASToken::on_rowsInserted);
...
void OASToken::setClientModel(ClientModel *_clientModel)
{
...
void OASToken::on_goToSendTokenPage()
{
    m_sendTokenPage->show();
}

void OASToken::on_goToReceiveTokenPage()
{
    m_receiveTokenPage->show();
}

void OASToken::on_goToAddTokenPage()
{
    m_addTokenPage->show();
}

void OASToken::on_currentTokenChanged(QModelIndex index)
{
...
void OASToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void OASToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void OASToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void OASToken::contextualMenu(const QPoint &point)
{
...
void OASToken::copyTokenAddress()
{
...
void OASToken::copyTokenBalance()
{
    if(indexMenu.isValid())
    {
...
void OASToken::copySenderAddress()
{
    if(indexMenu.isValid())
    {
...
void OASToken::on_sendToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToSendTokenPage();
}

void OASToken::on_receiveToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToReceiveTokenPage();
}

void OASToken::on_addToken()
{
    on_goToAddTokenPage();
}
