# bash programmable completion for odan-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $odan-cli for RPC
_odan_rpc() {
...
_odan_cli() {
    ...
    # save and use original argument to invoke odan-cli for -help, help and RPC
    # as odan-cli might not be in $PATH
    odan_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($odan_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_odan_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
