...
class RPCGenerateTest(BitcoinTestFramework):
    ...
    def test_generatetoaddress(self):
        self.generatetoaddress(self.nodes[0], 1, convert_btc_address_to_odan('mneYUmWYsuk7kySiURxCi3AGxrAqZxLgPZ'))
    ...
    def test_generateblock(self):
        ...
        combo_address = convert_btc_bech32_address_to_odan('bcrt1qw508d6qejxtdg4y5r3zarvary0c5xw7kygt080')
        ...
        combo_address = convert_btc_address_to_odan('mkc9STceoCcjoXEXe6cm66iJbmjM6zR9B2')
        hash = self.generateblock(node, 'combo(' + combo_key + ')', [])['hash']
        block = node.getblock(hash, 2)
        assert_equal(len(block['tx']), 1)
        assert_equal(block['tx'][0]['vout'][0]['scriptPubKey']['address'], combo_address)
