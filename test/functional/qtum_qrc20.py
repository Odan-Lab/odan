...
oasFburn
oasFburnfrom
oasFlisttransactions
"""


class OdanQRC20Test(BitcoinTestFramework):
    ...
    def oasFname_test(self):
        assert_equal(self.node.oasFname(self.contract_address), "QRC TEST")

    def oasFsymbol_test(self):
        assert_equal(self.node.oasFsymbol(self.contract_address), "QTC")

    def oasFtotalsupply_test(self):
        assert_equal(self.node.oasFtotalsupply(self.contract_address), f"{10**65}.00000000")

    def oasFdecimals_test(self):
        assert_equal(self.node.oasFdecimals(self.contract_address), 8)
    ...
    def run_test(self):
        ...
        """
            ...
            */
            contract QRC20Token is SafeMath {
                ...
                uint8 public constant decimals = 8; // it's recommended to set decimals to 8 in ODAN
        ...
        self.oasFname_test()
        self.oasFsymbol_test()
        self.oasFtotalsupply_test()
        self.oasFdecimals_test()
        assert_equal(self.node.oasFbalanceof(self.contract_address, self.creator), f"{10**65}.00000000")

        self.disconnect_nodes(0, 1)
        for n in self.nodes:
            assert_equal(n.getpeerinfo(), [])

        self.node.oasFtransfer(self.contract_address, self.creator, self.receiver, "0.00000001")
        self.node.generate(1)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.node.oasFlisttransactions(self.contract_address, self.creator, 0, 0), 
            [{
        ...
        assert_equal(self.node.oasFlisttransactions(self.contract_address, self.receiver, 0, 0), 
            [{
                'receiver': self.receiver, 
                'sender': self.creator,
                'amount': '0.00000001', 
        ...
        assert_equal(self.node.oasFbalanceof(self.contract_address, self.creator), f"{10**65-1}.99999999")
        assert_equal(float(self.node.oasFbalanceof(self.contract_address, self.receiver)), 0.00000001)


        # Double spend the oasF transaction to clear it from the mempool and reorg to clear the state
        self.double_spend_tx(self.node, tip['tx'][1], self.creator, self.reorg_node)
        self.reorg_node.generate(2)
        self.connect_nodes(0, 1)
        self.sync_blocks(self.nodes)
        assert_equal(self.node.oasFlisttransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.oasFlisttransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.oasFbalanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(self.node.oasFbalanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(float(self.reorg_node.oasFbalanceof(self.contract_address, self.receiver)), 0)
        assert_equal(float(self.node.oasFbalanceof(self.contract_address, self.receiver)), 0)
        ...
        self.node.oasFtransfer(self.contract_address, self.creator, self.receiver, f"{10**65}.00000000")
        self.node.generate(1)
        ...
        assert_raises_rpc_error(-1, "Not enough token balance", self.reorg_node.oasFtransfer, self.contract_address, self.receiver, self.creator, "0.1")

        # Make sure that sending from an address that we do not control fails gracefully
        assert_raises_rpc_error(-4, "Private key not available", self.reorg_node.oasFtransfer, self.contract_address, self.creator, self.receiver, "0.1")

        # Make sure that the allowance functionality works as intended
        self.approve_tx_receiver = self.node.getnewaddress()
        self.node.oasFapprove(self.contract_address, self.creator, self.receiver, "0.1")
        self.node.generatetoaddress(1, self.creator)
        self.sync_blocks()

        assert_equal(float(self.node.oasFallowance(self.contract_address, self.creator, self.receiver)), 0.1)
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.oasFtransferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.10000001")

        self.reorg_node.oasFtransferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.01")
        self.sync_all()
        self.node.generatetoaddress(1, self.creator)
        self.sync_blocks()
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.reorg_node.oasFlisttransactions(self.contract_address, self.approve_tx_receiver, 0, 0), [{
            'amount': '0.01000000',
            'blockHash': tip['hash'],
            'blockNumber': 2107,
            'blocktime': tip['time'],
        ...
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.oasFtransferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000001")

        self.reorg_node.oasFtransferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000000")
        ...
        assert_equal(float(self.node.oasFallowance(self.contract_address, self.creator, self.receiver)), 0)
        assert_equal(self.reorg_node.oasFbalanceof(self.contract_address, self.creator), f"{10**65-1}.90000000")
        assert_equal(self.node.oasFbalanceof(self.contract_address, self.approve_tx_receiver), f"0.10000000")
