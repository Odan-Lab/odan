...
from .odanconfig import COINBASE_MATURITY
from .odan import generatesynchronized
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "odand": ("bitcoind", "BITCOIND"),
            "odan-cli": ("bitcoincli", "BITCOINCLI"),
            "odan-util": ("bitcoinutil", "BITCOINUTIL"),
            "odan-wallet": ("bitcoinwallet", "BITCOINWALLET"),
        }
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'odand', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'odan-cli', self.options.bitcoincli) for v in versions]
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            for entry in os.listdir(cache_path()):
                if entry not in ['chainstate', 'blocks', 'indexes', 'stateOdan']:  # Only indexes, chainstate and blocks folders
