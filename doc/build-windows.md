...
Below are some notes on how to build Odan Core for Windows.

The options known to work for building Odan Core on Windows are:
...
Note that for WSL the Odan Core source path MUST be somewhere in the default mount file system, for
example /usr/src/odan, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\odan`, for example:

    make install DESTDIR=/mnt/c/workspace/odan
