UNIX BUILD NOTES
====================
Some notes on how to build Odan Core in Unix.
...
memory available when compiling Odan Core. On systems with less, gcc can be
...
To build Odan Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build odan-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a odan-qt executable will be
built by default.
...
You need to build boost manually, and if it's not in standard library paths, you need to add `/path/to/boost/lib` into `LD_LIBRARY_PATH` env when building Odan.
...
Build requirements:
```
./odan/contrib/script/setup-ubuntu16.sh
```

#### Build Installation Package
Build Odan:
```
cd odan/contrib/script
./build-odan-linux.sh -j2
```
The home folder for the installation package need to be `odan/contrib/script`.
After the build finish, the installation package is present into `odan/contrib/script`.
Installation package example: `odan-22.1-x86_64-pc-linux-gnu.tar.gz`

#### Dependencies Installation Package

The package has the following dependencies when used on Ubuntu 16 machine that is not used for building Odan:

`odan-qt` require `libxcb-xinerama0` to be installed on Ubuntu 16 (both 32 and 64 bit versions):
```
sudo apt-get install libxcb-xinerama0 -y
```

Odan require `GCC 7` standard library be installed for Ubuntu 16 only on 32 bit version:
```
...
```
su
./odan/contrib/script/setup-centos7.sh
```
...
...
to: /path/to/odan/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/odan/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Odan Core can
...
This example lists the steps necessary to setup and build a command line only distribution of the latest changes on Arch Linux:
    ...
    git clone https://github.com/Odan-Lab/odan --recursive
    cd odan/
    ./autogen.sh
    ./configure
    make check
